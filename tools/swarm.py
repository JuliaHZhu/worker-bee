...
def _nats_connect_kwargs() -> dict:
    """Build connection kwargs for nats.connect including optional auth."""
    kwargs = {}
    if NATS_USER:
        kwargs["user"] = NATS_USER
        kwargs["password"] = NATS_PASSWORD
    return kwargs
...
async def _publish(subject: str, payload: dict):
    import nats
    nc = await nats.connect(NATS_URL, **_nats_connect_kwargs())
    try:
        js = nc.jetstream()
        envelope = {
