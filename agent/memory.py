...
class SessionDB:
    ...
    def export_handoff(self, session_id: str, out_path: str | None = None) -> str:
        ...
        def _escape_md(text: str) -> str:
            """Escape leading '#' so user text cannot inject markdown headers."""
            # Prefix every line that starts with '#' with a backslash
            return "\n".join(
        ...
        if purpose:
            lines.extend(["## Purpose", _escape_md(purpose), ""])
        ...
        if todos:
            ...
            for tid, content, status, _ in todos:
                ...
                lines.append(f"- {mark} {_escape_md(content)}")
            lines.append("")
        ...
        if recent_user_msgs:
            ...
            lines.extend(["## Next Step", _escape_md(recent_user_msgs[-1]), ""])
