...
auto_detect_role() {
  ...
  case "$ip" in
    *) python3 -c "import sys; sys.path.insert(0,"."); from beebox.nodes import self_role; print(self_role())" 2>/dev/null || echo "unknown" ;;
    *) echo "unknown" ;;
  esac
}
...
port: 4222
jetstream: {
  store_dir: "$HOME/.worker-bee/nats-jetstream"
