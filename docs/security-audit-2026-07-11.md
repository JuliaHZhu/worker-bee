...
> 审计日期：2026-07-11  
> 审计 commit：`eeca1b0`  
> 审计范围：全仓库逐文件审查（`agent/` `tools/` `gateway/` `swarm/` `beebox/` `scripts/` `cron/`）
...
| 文件 | 行号 | 泄露内容 |
|------|------|---------|
| `gateway/run.py` | 21 | `DEFAULT_SWARM_NATS_URL = "nats://43.156.129.115:4222"` |
| `scripts/swarm-node.sh` | 8 | `PM_IP="43.156.129.115"` |
...
- `gateway/run.py` 默认值改 `localhost`，生产值通过环境变量注入

---

### 2. NATS 集群零认证 + 零加密 + 公网监听

**NATS Server 配置现状**（`swarm/server.conf`、`DEPLOY-CHECKLIST.md:51-59`、`swarm-node.sh:91-99`）：

- 端口 4222（客户端接入）：**无认证，无 TLS**
- 端口 6222（集群通信）：`listen: 0.0.0.0`，**无认证**
- 端口 8222（HTTP 监控）：**暴露**
- `swarm/server.conf:6` 注释自证："生产环境：加上 authorization 和 TLS / 内网开发：裸跑即可"
...
`gateway/platforms/feishu.py:125`：
```python
...
`gateway/config.py:21`：`host: str = "0.0.0.0"` — 除非显式覆盖，Feishu webhook 接收器绑定所有网卡。虽然后续有 token 验证（问题 4 削弱了它），但端口 8080 的 HTTP 服务本身公网可达。
...
- `gateway/platforms/feishu.py:23-25` — `_feishu_token` + `_feishu_token_expires` + `_token_lock`（这个有锁，但前两个没有）
...
| `gateway/platforms/feishu.py` | 飞书 webhook 有 token 验证 + challenge handshake | 良好（除问题 4） |
