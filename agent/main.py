...
from agent.tick import _tick_stop, _tick_thread, _cron_tick_loop
...
def setup():
    ...
    logger.info("  [1] OpenAI-compatible / Moonshot (OpenAI protocol)")
    logger.info("  [2] Anthropic / Volcano (Anthropic protocol)")
    p = input("> ").strip()
    if p == "2":
        provider = "anthropic"
    ...
    if not agent_md.exists():
        ...
        logger.info(f"✅ Created {agent_md}")
    else:
        logger.info(f"⏭️  {agent_md} already exists — skipped")

    soul_md = prompt_dir / "soul.md"
    if not soul_md.exists():
        soul_md.write_text(_SOUL_MD_TEMPLATE, encoding="utf-8")
        logger.info(f"✅ Created {soul_md}")
    else:
    ...
    logger.info(f"✅ Verified: {agent_size} chars loaded from agent.md, {soul_size} chars from soul.md")
    logger.info("   Next worker-bee run will inject them into the system prompt.")
...
def ping_model(message: str, temperature: float | None = None):
    ...
    config = load_config()
    if not config:
        logger.info("❌ No config. Run: worker-bee setup")
        sys.exit(1)
    if temperature is not None:
        config["temperature"] = temperature
    logger.info(f"→ Pinging {config['model']} ({config['provider']}) temp={config.get('temperature', 0.0)}...")
    try:
...
def config_cmd(args: list[str]):
    ...
    config = load_config()
    if not config:
        logger.info("❌ No config found. Run: worker-bee setup")
        sys.exit(1)
    ...
    # Set a key
    if len(args) != 2:
        logger.info("Usage: worker-bee config [key value]")
        logger.info("   or: worker-bee config")
        sys.exit(1)

    key, value = args[0], args[1]
    if key not in config:
        logger.info(f"⚠️ Unknown key '{key}'. Available: {', '.join(config.keys())}")
        sys.exit(1)
    ...
    if isinstance(original, bool):
        ...
        try:
            ...
            logger.info(f"❌ Cannot convert '{value}' to {type(original).__name__}")
            sys.exit(1)
    else:
        config[key] = value
...
def ping_channel(message: str):
    ...
    if app_id and app_secret:
        ...
        if target:
            logger.info(f"→ Sending via Feishu App Bot API to {target[:20]}...")
            result = send_message(
                message,
                platform="feishu",
                receive_id=target,
            ...
            logger.info("⚠️  Feishu App Bot credentials found but no target chat_id.")
            logger.info("   Set FEISHU_HOME_CHANNEL or run inside Hermes Feishu context.")
            return
    ...
    if plat == "feishu":
        logger.info("→ Sending via Feishu Webhook...")
        result = send_message(message)
        logger.info("← Result:", result)
        return

    if plat == "discord":
        logger.info("→ Sending via Discord Webhook...")
        result = send_message(message)
        logger.info("← Result:", result)
        return

    # 3. Nothing configured
    logger.info("❌ No messaging platform configured.")
    logger.info("   Options:")
    logger.info("   • App Bot (full):  export FEISHU_APP_ID='xxx' FEISHU_APP_SECRET='yyy'")
    logger.info("   • Webhook (simple): export FEISHU_WEBHOOK_URL='https://open.feishu.cn/...'")
    logger.info("   • Discord:          export DISCORD_WEBHOOK_URL='https://discord.com/...'")
...
def run_session(temperature_override: float | None = None):
    ...
    logger.info("Commands: /exit, /history, /tools, /clear, /todo, /skills, /cats, /infra, /export")
    logger.info("-" * 50)
    ...
    while True:
        ...
        if user_input.lower() == "/history":
            for m in st.messages[-10:]:
                ...
                logger.info(f"  [{role:10}] {content}...{tag_str}")
            continue
        ...
        if user_input.lower() == "/cats":
            for cat, names in sorted(registry.list_by_category().items()):
                logger.info(f"  {cat}: {len(names)} tool(s)")
            continue
        if user_input.lower() == "/infra":
            logger.info(st.infra.describe())
            continue
        ...
        if user_input.lower().startswith("/task"):
            logger.info("⚠️  /task is deprecated. Use natural language or /todo instead.")
            continue
        if user_input.lower() == "/skills":
            ...
            if skills:
                for name, meta in skills.items():
                    ...
                    logger.info(f"  • {name}: {meta.get('description', 'No description')}{trig_str}{tool_str}")
            else:
                logger.info("No skills loaded.")
            continue
        if user_input.lower() == "/export":
            path = st.db.export_handoff(st.session_id)
            logger.info(f"Handoff exported to: {path}")
            logger.info("  Start a new session with: worker-bee --continue <path>")
            continue
        ...
        # --- Deck procurement ---
        logger.info("  [Procuring deck...]", flush=True)
        ...
        logger.info(f"  [Deck ready: {deck.size()} tools]  (mode: {st.deck_mgr.mode})")
        if matched_skills:
            logger.info(f"    Primary skills: {', '.join(matched_skills)}")
        if extended_skills:
            logger.info(f"    Extended skills: {', '.join(extended_skills)}")
        ...
        if response == "(reached max iterations)":
            ...
            logger.info("   Please rephrase your request or check /tools and /skills.")
...
def _handle_todo(cmd: str, db: SessionDB, session_id: str):
    ...
    if len(parts) == 1:
        ...
        for tid, content, status, created in todos:
            ...
            logger.info(f"  {mark} [{tid}] {content}")
    elif parts[1] == "add" and len(parts) == 3:
        tid = db.add_todo(session_id, parts[2])
        logger.info(f"Added todo [{tid}].")
    elif parts[1] == "done" and len(parts) == 3:
        try:
            db.update_todo_status(int(parts[2]), "done")
            logger.info(f"Marked todo {parts[2]} as done.")
        except ValueError:
            logger.info("Usage: /todo done <id>")
    elif parts[1] == "pending" and len(parts) == 3:
        try:
            db.update_todo_status(int(parts[2]), "pending")
            logger.info(f"Marked todo {parts[2]} as pending.")
        except ValueError:
            logger.info("Usage: /todo pending <id>")
    elif parts[1] == "delete" and len(parts) == 3:
        try:
            db.delete_todo(int(parts[2]))
            logger.info(f"Deleted todo {parts[2]}.")
        except ValueError:
            logger.info("Usage: /todo delete <id>")
    else:
        logger.info("Usage: /todo, /todo add <text>, /todo done <id>, /todo pending <id>, /todo delete <id>")
...
def _handle_deck(cmd: str, deck_mgr: DeckManager):
    ...
    if len(parts) == 1:
        ...
        logger.info(f"Mode: {deck_mgr.mode}")
        logger.info(f"Tools ({len(tools)}): {', '.join(tools) if tools else '(none)'}")
        return
    sub = parts[1].lower()
    if sub == "mode":
        logger.info(f"Current mode: {deck_mgr.mode}")
    elif sub == "full":
        logger.info(deck_mgr.set_mode("full"))
    elif sub == "focus":
        logger.info(deck_mgr.set_mode("focus"))
    elif sub == "add" and len(parts) == 3:
        logger.info(deck_mgr.add_tool(parts[2]))
    elif sub == "drop" and len(parts) == 3:
        logger.info(deck_mgr.drop_tool(parts[2]))
    elif sub == "reset":
        logger.info(deck_mgr.reset())
    elif sub == "list":
        tools = deck_mgr.list_tools()
        logger.info(f"Tools ({len(tools)}): {', '.join(tools) if tools else '(none)'}")
    elif sub == "log":
        ...
        logger.info("Usage: /deck [mode|full|focus|add <tool>|drop <tool>|reset|list|log]")
...
""")
    ...
    if args.command == "retry-rate-limited":
        ...
        if not pending:
            logger.info("API 正常，无待恢复任务。")
            return
        retried = retry_rate_limited_jobs()
        logger.info(f"已恢复 {len(retried)} 个因限流暂停的任务，下个 tick 执行。")
        return
